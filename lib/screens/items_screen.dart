       ...
       Future.delayed(Duration.zero, () async {
         productController.geSubtCategory(parentcatid,onload: (status){
           ...
           ProductRepo().getSubNestcategory(subcatId.toString()).then((value) {
             ...
             _displayitem(subcatId.toString(), ListofNest.length>0?int.parse("0"):(_nosub == "true"&& VxState.store.homescreen.data!.allCategoryDetails![index].categoryType == 0)?int.parse("0"):int.parse(/*"0"*/indvalue!)+1,0,"0");
           });
       });
     });
   });
